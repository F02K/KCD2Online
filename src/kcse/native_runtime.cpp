...
namespace kcd2o::kcse
{
	namespace
	{
		constexpr auto environment_apply_result = "KCD2Online_EnvironmentApplied";
		constexpr auto environment_time_offset  = "KCD2Online_WorldTimeOffset";
		...
		bool read_system_global_state(std::uint32_t &state) noexcept
		{
			...
			__except (KCD2Online_JOIN_SEH_FILTER("join.world.GetSystemGlobalState.seh"))
			{
				return false;
			}
#endif
		...
		wh::guimodule::C_UIMenu *find_main_menu() noexcept
		{
			...
			for (const auto &element : module->m_uiElements)
			{
				...
				{
					return reinterpret_cast<wh::guimodule::C_UIMenu *>(candidate);
				}
			}
			return nullptr;
		}
		...
		wh::guimodule::C_UIMenu *guarded_find_main_menu() noexcept
		{
			...
			__except (KCD2Online_JOIN_SEH_FILTER("join.world.FindMainMenu.seh"))
			{
				return nullptr;
			}
#else
		...
		bool hide_game_over() noexcept
		{
			...
			{
				auto *candidate = element.get();
				if (!candidate || *reinterpret_cast<const std::uintptr_t *>(candidate) != game_over_vtable)
				{
					continue;
				}

				auto *game_over = reinterpret_cast<wh::guimodule::C_UIGameOver *>(candidate);
				auto *interface = static_cast<wh::playermodule::I_UIGameOver *>(game_over);
				auto **vtable   = *reinterpret_cast<void ***>(interface);
				if (!vtable || reinterpret_cast<std::uintptr_t>(vtable) != game_over_interface_vtable || !in_whgame_text(vtable[1]))
				{
					return false;
				}
				interface->Hide();
		...
		bool guarded_hide_game_over() noexcept
		{
			...
			__except (KCD2Online_JOIN_SEH_FILTER("join.respawn.HideGameOver.seh"))
			{
				return false;
			}
#else
		...
		bool start_debug_new_game(wh::guimodule::C_UISaveLoad *save_load, const char *level_name) noexcept
		{
			...
			using start_new_game = void(__fastcall *)(wh::guimodule::C_UISaveLoad *, int, CryStringT<char>, const char *);
			const auto address = REL::ID(141'236).address();
			if (!in_whgame_text(reinterpret_cast<const void *>(address)))
			{
				return false;
			}
			reinterpret_cast<start_new_game>(address)(save_load, 1, CryStringT<char>{}, level_name);
			return true;
		}

		bool guarded_start_debug_new_game(wh::guimodule::C_UISaveLoad *save_load, const char *level_name) noexcept
		{
#ifdef _WIN32
			__try
			{
		...
		{
			const auto length_squared = rotation->x() * rotation->x() + rotation->y() * rotation->y()
		...
		bool execute_console_command(const char *command, bool deferred = false) noexcept
		{
			auto *environment = SSystemGlobalEnvironment::GetInstance();
		...
		bool execute_script(std::string_view script) noexcept
		{
			...
			{
#endif
				return environment->pScriptSystem->ExecuteBuffer(script.data(), script.size(), "KCD2Online multiplayer rule", nullptr);
#ifdef _WIN32
			}
			__except (EXCEPTION_EXECUTE_HANDLER)
			{
		...
		player_respawn_identity capture_player_respawn_identity(const native_player_view &player) noexcept
		{
			...
			__except (KCD2Online_JOIN_SEH_FILTER("join.respawn.CapturePlayerIdentity.seh"))
			{
				return {};
			}
#endif
		...
		std::optional<std::uint64_t> read_home_marker_wuid() noexcept
		{
			...
			{
#endif
				const auto read = environment->pScriptSystem->GetGlobalAny("KCD2Online_HomeMarkerWUID", value);
				environment->pScriptSystem->SetGlobalToNull("KCD2Online_HomeMarkerWUID");
		...
		wh::xgenaimodule::C_LinkableObject *find_linkable_object_by_wuid(std::uint64_t wuid) noexcept
		{
			if (wuid == 0)
			{
				return nullptr;
			...
			__try
			{
				...
				auto *manager = module && module->m_pSingletons ?
				                    static_cast<wh::xgenaimodule::C_AIObjectManager *>(module->m_pSingletons->GetAIObjectManager()) :
				                    nullptr;
				if (!manager)
				{
					return nullptr;
				}
				const auto found = manager->m_objects.find(wh::framework::WUID{wuid});
				if (found == manager->m_objects.end() || !found->second)
				{
					return nullptr;
				}
				return kcd_cast<wh::xgenaimodule::C_LinkableObject *>(found->second);
#ifdef _WIN32
			}
		...
		wh::xgenaimodule::C_LinkableObject *find_linkable_object(std::uint32_t entity_id)
		{
			if (entity_id == 0)
			{
				return nullptr;
			...
			const auto query = std::format("KCD2Online_HomeMarkerWUID=nil; do local e=System.GetEntity({}); "
		...
		bool apply_home_marker_native(const protocol::PropertyHomeMarker &marker, home_marker_apply_result &result)
		{
			auto *environment = SSystemGlobalEnvironment::GetInstance();
			...
			result.entity_id = environment->pEntitySystem->FindEntityByGuid(marker.entity_guid());
			...
			result.map->GetOrCreateEntityMark(result.mark, wh::guimodule::E_MarkType::Home, object, 2);
			if (!result.mark)
			{
				return false;
			}
			result.filter_was_visible = result.map->IsPoiFilterVisible(wh::guimodule::E_MarkType::Home);
			result.map->RegisterEntityMark(result.mark);
			if (!result.filter_was_visible)
			{
				result.map->SetPoiMarkersVisible(wh::guimodule::E_MarkType::Home, true);
			}
			return true;
		}

		bool guarded_apply_home_marker(const protocol::PropertyHomeMarker &marker, home_marker_apply_result &result) noexcept
		{
#ifdef _WIN32
			__try
			{
		...
		void remove_home_entity_mark(wh::guimodule::C_UIMap *map, std::shared_ptr<wh::guimodule::S_EntityMapMark> &mark)
		{
			// Native C_UIMap::RemoveEntityMark, the same release path used by
			// C_ShowMapMarker::Untrigger.
			using function = void(__fastcall *)(wh::guimodule::C_UIMap *, std::shared_ptr<wh::guimodule::S_EntityMapMark> *);
			static REL::Relocation<function> native{REL::ID(55'367)};
			native(map, &mark);
		}
		...
		{
			auto *environment = SSystemGlobalEnvironment::GetInstance();
			return environment && environment->pConsole && environment->pScriptSystem;
		}
		...
		protocol::Quaternion quaternion_from_matrix(const Matrix34 &matrix)
		{
			...
			{
				const auto scale = std::sqrt(1.0F + matrix.m00 - matrix.m11 - matrix.m22) * 2.0F;
			...
			{
				const auto scale = std::sqrt(1.0F + matrix.m11 - matrix.m00 - matrix.m22) * 2.0F;
			...
			{
				const auto scale = std::sqrt(1.0F + matrix.m22 - matrix.m00 - matrix.m11) * 2.0F;
		...
		protocol::Vec3 facing_from_rotation(const protocol::Quaternion &rotation)
		{
			protocol::Vec3 result;
			result.set_x(2.0F * (rotation.x() * rotation.y() - rotation.w() * rotation.z()));
			result.set_y(1.0F - 2.0F * (rotation.x() * rotation.x() + rotation.z() * rotation.z()));
			result.set_z(2.0F * (rotation.y() * rotation.z() + rotation.w() * rotation.x()));
			return result;
		}
	    ...
	    m_remote_avatars(m_remote_backend)
	{
		...
		m_address_library_distribution = std::string(REL::to_string(address_library.distribution));
		std::ranges::transform(m_address_library_distribution,
		...
		m_diagnostic   = "Waiting for KCSE PostUpdate; no savegame is required.";
	}
	...
	{
		++m_frame_sequence;
		KCD2Online_JOIN_TRACE("join.kcse.frame.begin",
		                      std::format("epoch={} invalidated={} data_loaded={}",
		...
		m_voice.tick();
		const auto changed = m_epoch_invalidated.exchange(false, std::memory_order_acq_rel);
		if (changed)
		{
		...
		KCD2Online_JOIN_TRACE("join.kcse.frame.complete", std::format("epoch_changed={} unload_transition={}", changed, unload_transition));
		return changed && !unload_transition;
	}

	void native_runtime::on_blacksmithing_started(std::uint32_t station_entity_id)
	{
		...
		auto *environment = SSystemGlobalEnvironment::GetInstance();
		auto *station = environment && environment->pEntitySystem ? environment->pEntitySystem->GetEntity(station_entity_id) : nullptr;
		if (!station || station->GetGuid() == 0)
		{
			return;
		}
		m_pending_activity_start = local_activity_start{protocol::PLAYER_ACTIVITY_KIND_BLACKSMITHING, station->GetGuid()};
		m_native_activity_kind = protocol::PLAYER_ACTIVITY_KIND_BLACKSMITHING;
		m_activity_end_pending = false;
		KCD2Online_JOIN_TRACE("activity.blacksmithing.started",
		                      std::format("station_entity_id={} station_guid={}", station_entity_id, station->GetGuid()));
	}

	std::optional<local_activity_start> native_runtime::take_local_activity_start()
	{
	...
	wh::playermodule::I_Minigame *native_runtime::find_local_minigame(protocol::PlayerActivityKind kind) const
	{
		...
		case protocol::PLAYER_ACTIVITY_KIND_SHARPENING:
			...
			break;
		case protocol::PLAYER_ACTIVITY_KIND_ALCHEMY: native_kind = wh::playermodule::E_MinigameType::Alchemy; break;
		case protocol::PLAYER_ACTIVITY_KIND_NONE:
		default:                                     return nullptr;
		}
		auto *context = wh::game::S_GameContext::GetInstance();
		auto player   = m_entities.player();
		if (!context || !context->m_pPlayerModule || !context->m_pPlayerModule->m_pMinigameManager || !player.entity)
		{
			return nullptr;
		}
		return context->m_pPlayerModule->m_pMinigameManager->FindOrCreateSession(player.entity->GetId(), native_kind, false, false);
	}

	void native_runtime::poll_local_activity()
	{
		if (m_native_activity_kind == protocol::PLAYER_ACTIVITY_KIND_NONE || m_activity_end_pending)
		{
			return;
		}
		...
		KCD2Online_JOIN_TRACE("activity.local.finished", std::format("kind={}", static_cast<int>(m_native_activity_kind)));
		m_native_activity_kind = protocol::PLAYER_ACTIVITY_KIND_NONE;
	...
	{
		std::scoped_lock lock(m_cache_mutex);
		return {m_capabilities | known_client_runtime_capabilities,
	...
	runtime_gate native_runtime::capability() const
	{
		...
		{
			return {false, false, "Multiplayer runtime is idle; click Connect to initialize it."};
		}
		const auto address_library_ready = !m_address_library.empty() && !m_address_library_distribution.empty() && m_address_library_format != 0 && m_address_library_entries != 0
		                                   && m_address_library_sha256.size() == 64;
		if (m_frame_seen.load(std::memory_order_acquire) && address_library_ready && !m_unload_pending)
		{
			return {true, false, {}};
		}
		return {false, true, "Waiting for the KCSE game-thread and Address Library bootstrap."};
	}

	bool native_runtime::can_start_join() const
	{
		std::scoped_lock lock(m_cache_mutex);
		return m_frame_seen.load(std::memory_order_acquire) && !m_address_library.empty()
		       && !m_address_library_distribution.empty() && m_address_library_format != 0 && m_address_library_entries != 0
		       && m_address_library_sha256.size() == 64 && !m_unload_pending;
	}

	bool native_runtime::prepare_multiplayer()
	{
		KCD2Online_JOIN_TRACE("join.runtime.prepare.precheck",
		                      std::format("data_loaded={} frame_seen={} can_start_join={}",
		                                  m_data_loaded.load(std::memory_order_acquire),
		...
		{
			std::scoped_lock lock(m_cache_mutex);
			m_diagnostic = "KCSE has not reached a safe game-thread frame yet.";
			KCD2Online_JOIN_TRACE("join.runtime.prepare.rejected", m_diagnostic);
			return false;
		}
		m_multiplayer_requested.store(true, std::memory_order_release);
		{
			std::scoped_lock lock(m_cache_mutex);
			m_diagnostic = m_local_transform ? "Initializing native multiplayer runtime capabilities." : "Connecting before world creation; awaiting server bootstrap.";
		}
		KCD2Online_JOIN_TRACE("join.runtime.prepare.requested", std::format("epoch={}", m_epoch.load(std::memory_order_acquire)));
		return true;
	}

	void native_runtime::cancel_multiplayer_preparation()
	{
		if (!m_multiplayer_requested.exchange(false, std::memory_order_acq_rel) && !m_preparation_active)
		{
			return;
		}
		remove_home_marker();
		...
		{
			m_sandbox_progress = {};
			m_diagnostic       = "Multiplayer runtime is idle; click Connect to initialize it.";
		}
	}

	sandbox_start_result native_runtime::begin_sandbox(const protocol::ServerBootstrap &bootstrap)
	{
		const auto target = find_native_world_level(bootstrap.level_id());
		if (!target)
		{
			return {false, "Server level '" + bootstrap.level_id() + "' is not registered in the native KCD2 level catalog."};
		}
		...
		{
			std::scoped_lock lock(m_cache_mutex);
			loaded_target = m_data_loaded.load(std::memory_order_acquire) && m_level_id == target->id;
		...
		if (loaded_target && has_player)
		{
			...
			m_diagnostic                     = "Loaded target world is waiting for native capability validation.";
			KCD2Online_JOIN_TRACE("join.world.loaded-probe", std::format("target_level={} missing=0x{:X}", target->id, required_client_runtime_capabilities & ~capabilities));
			return {true, {}};
		}
		if (has_player)
		{
	...
	sandbox_start_result native_runtime::activate_loaded_sandbox(const protocol::ServerBootstrap &bootstrap)
	{
		KCD2Online_JOIN_TRACE("join.sandbox.native.begin",
		                      std::format("session_id=\"{}\" requested_level=\"{}\" mode={} "
		                                  ...
		                                  bootstrap.has_profile() && bootstrap.profile().transform_valid(),
		                                  bootstrap.has_profile() && bootstrap.profile().has_last_transform(),
		                                  current_level_id(),
		                                  descriptor().capabilities));
		std::unique_lock lock(m_cache_mutex);
		if (!m_local_transform)
		{
			KCD2Online_JOIN_TRACE("join.sandbox.native.rejected", "local transform is nil");
		...
		{
			KCD2Online_JOIN_TRACE("join.sandbox.native.rejected",
			                      std::format("level mismatch requested=\"{}\" current=\"{}\"", bootstrap.level_id(), m_level_id));
			return {false, "Loaded native world does not match the server level."};
		}
		if ((m_capabilities & runtime_capability_profile_apply) == 0 || !bootstrap.has_profile())
		{
			KCD2Online_JOIN_TRACE("join.sandbox.native.rejected",
			                      std::format("profile_apply_capability={} bootstrap_profile={}",
		...
		const auto selected_spawn = select_sandbox_spawn(bootstrap, m_local_transform);
		if (!selected_spawn.transform)
		{
			KCD2Online_JOIN_TRACE("join.sandbox.spawn-selection.failed",
			                      std::format("mode={} profile_transform_valid={} "
			...
			return {false, "Server bootstrap contains no usable spawn transform."};
		}
		const auto &spawn       = *selected_spawn.transform;
		const auto spawn_source = to_string(selected_spawn.source);
		KCD2Online_JOIN_TRACE("join.sandbox.spawn-selection.ok",
		                      std::format("source={} position=({:.6f},{:.6f},{:.6f}) "
		...
		{
			KCD2Online_JOIN_TRACE("join.sandbox.spawn-selection.invalid", std::format("source={}", spawn_source));
			return {false, "Selected sandbox spawn transform is invalid."};
		}
		...
		*target.mutable_last_transform() = spawn;
		...
		{
			KCD2Online_JOIN_TRACE("join.sandbox.target-profile.invalid",
			                      std::format("source={} player_id={} revision={} level=\"{}\" "
		...
		KCD2Online_JOIN_TRACE("join.sandbox.target-profile.valid",
		                      std::format("player_id={} revision={} source={}", target.player_id(), target.revision(), spawn_source));

		auto *framework = CCryAction::GetInstance();
		if (!framework)
		{
			KCD2Online_JOIN_TRACE("join.sandbox.native.rejected", "CCryAction=nil");
			return {false, "CCryAction is unavailable."};
		}
		m_native_weather_revision = 0;
		lock.unlock();
		clear_script_global(environment_time_offset);
		const auto environment_applied = apply_environment_state(bootstrap.environment(), true);
		lock.lock();
		if (!environment_applied)
		{
			const auto environment_error =
			    m_diagnostic.empty() ? std::string{"Native server environment bootstrap failed."} : m_diagnostic;
			KCD2Online_JOIN_TRACE("join.sandbox.environment.failed", environment_error);
			return {false, environment_error};
		}
		KCD2Online_JOIN_TRACE("join.sandbox.environment.ok",
		                      std::format("time_of_day={} time_scale={} weather_id={} "
		...
		KCD2Online_JOIN_TRACE("join.sandbox.save-load-lock.begin", std::format("framework={}", static_cast<void *>(framework)));
		...
		KCD2Online_JOIN_TRACE(applied.success ? "join.sandbox.profile-apply.ok" : "join.sandbox.profile-apply.failed",
		                      std::format("success={} rollback_attempted={} rollback_succeeded={} "
		...
		if (!applied.success)
		{
			...
			{
				lock.unlock();
				begin_native_unload("Native profile rollback failed; unloading the modified "
				                    "save.");
			}
			else
			{
			...
			return {false, "Native profile transaction failed: " + applied.error};
		}
		...
		(void)execute_console_command("cheat_no_lockpicking nolockpicks:true");
		...
		for (const auto &object : bootstrap.world_objects())
		{
			if (!m_entities.apply_world_object_state(object, world_error))
			{
				...
				lock.unlock();
				begin_native_unload(world_error);
				return {false, "Native world-object bootstrap failed: " + world_error};
			}
		}
		for (const auto &item : bootstrap.world_items())
		{
			if (!m_entities.apply_world_item_state(item, world_error))
			{
				...
				lock.unlock();
				begin_native_unload(world_error);
				return {false, "Native world-item bootstrap failed: " + world_error};
			}
		}
		m_sandbox_active                 = true;
		const auto map_reveal_dispatched = execute_console_command("player_revealFow");
		KCD2Online_JOIN_TRACE(map_reveal_dispatched ? "join.sandbox.map-reveal.ok" : "join.sandbox.map-reveal.failed", map_reveal_dispatched ? "The multiplayer map fog was cleared." : "The multiplayer map fog reveal command could not be dispatched.");
		(void)execute_script("if player and player.EnableFastTravel then "
		...
		KCD2Online_JOIN_TRACE(
		    ...
		    std::format("spawn=({:.6f},{:.6f},{:.6f})", spawn.position().x(), spawn.position().y(), spawn.position().z()));
		return {true, {}};
	}

	sandbox_start_result native_runtime::begin_native_world_start(const protocol::ServerBootstrap &bootstrap)
	{
		const auto target = find_native_world_level(bootstrap.level_id());
		if (!target)
		{
		...
		{
			return {false, "The native main-menu New Game controller is not ready."};
		}

		{
			...
			m_diagnostic                     = "Invoking native New Game for level " + m_world_start_level_name + ".";
		}
		...
		KCD2Online_JOIN_TRACE(
		    ...
		    std::format("level_id={} level_name={} menu={} save_load={} rel_id=141236", target->id, target->name, static_cast<void *>(menu), static_cast<void *>(&menu->m_saveLoad)));
		if (!guarded_start_debug_new_game(&menu->m_saveLoad, m_world_start_level_name.c_str()))
		{
			fail_native_world_start("The native Warhorse New Game entry rejected the request.");
			return {false, poll_sandbox().error};
		}
		set_world_start_stage(world_start_stage::waiting_for_lifecycle, "Native New Game accepted; waiting for lifecycle events.");
		return {true, {}};
	}
	...
	void native_runtime::advance_native_world_start()
	{
		...
		if (stage == world_start_stage::failed || stage == world_start_stage::activating)
		{
			return;
		}
		if (requires_lifecycle && !m_world_lifecycle_seen.load(std::memory_order_acquire))
		{
			set_world_start_stage(world_start_stage::waiting_for_lifecycle, "Waiting for native NewGame/PreDataLoaded lifecycle events.");
			return;
		}
		if (!m_data_loaded.load(std::memory_order_acquire))
		{
			set_world_start_stage(world_start_stage::waiting_for_data, "Native loading is active; waiting for DataLoaded.");
			return;
		}
		...
		{
			set_world_start_stage(world_start_stage::waiting_for_level, "DataLoaded fired; waiting for wh_sys_BaseLevelId=" + target_id + ".");
			return;
		}
		if (!level_load_complete)
		{
			set_world_start_stage(world_start_stage::waiting_for_level, "Target level is present; waiting for the engine to report LEVEL_LOAD_COMPLETE.");
			return;
		}
		if (!player_ready || !transition_safe)
		{
			set_world_start_stage(world_start_stage::waiting_for_player, "Target level is loaded; waiting for native player modules.");
			return;
		}
		if (probe_failed)
		{
			fail_native_world_start(probe_error.empty() ? "Native multiplayer readiness probe failed after New Game." : probe_error);
			return;
		}
		const auto missing = required_client_runtime_capabilities & ~capabilities;
		if (missing != 0)
		{
			set_world_start_stage(world_start_stage::probing_runtime, std::format("Player is ready; validating native capabilities (missing 0x{:X}).", missing));
			return;
		}

		set_world_start_stage(world_start_stage::activating, "Native world is ready; applying authoritative server state.");
	...
	void native_runtime::set_world_start_stage(world_start_stage stage, std::string diagnostic)
	{
		bool changed{};
		{
			std::scoped_lock lock(m_cache_mutex);
	...
	void native_runtime::end_sandbox(std::string_view error)
	{
		...
		bool cancel_preparation_only{};
		{
			...
			{
				// A failed activation that rolled the native profile back has not
				// taken ownership of the current world. Unloading that world here
				// destroys the local Player while CryEngine still updates it. The
				// 2026-08-13 crash dump proved the resulting null Actor read in
				// WHGame.dll. Keep the current world and only cancel the join.
				cancel_preparation_only = true;
			}
			should_unload = !cancel_preparation_only;
			if (should_unload)
			{
		...
		if (should_unload)
		{
			begin_native_unload(error.empty() ? "Native sandbox world unload is in progress." : error);
		}
	}
	...
	std::optional<protocol::PlayerProfile> native_runtime::local_profile()
	{
		...
		{
			std::scoped_lock lock(m_cache_mutex);
			m_diagnostic = error.empty() ? "Native profile capture failed." : std::move(error);
		}
		return result;
	}

	bool native_runtime::set_npc_entities_disabled(bool humans_disabled, bool animals_disabled)
	{
		std::string error;
		const auto result = m_entities.set_world_isolated(humans_disabled, animals_disabled, error);
		if (!result)
		{
			std::scoped_lock lock(m_cache_mutex);
	...
	std::vector<protocol::WorldObjectState> native_runtime::poll_world_object_updates()
	{
	...
	bool native_runtime::apply_world_object_state(const protocol::WorldObjectState &state)
	{
		std::string error;
	...
	std::vector<protocol::WorldItemState> native_runtime::poll_world_item_updates()
	{
		return m_entities.poll_world_item_updates();
	}

	bool native_runtime::apply_world_item_state(const protocol::WorldItemState &state)
	{
		std::string error;
	...
	std::vector<protocol::NpcObservation> native_runtime::poll_npc_observations()
	{
		return m_entities.poll_npc_observations();
	}

	bool native_runtime::apply_npc_state(const protocol::NpcState &state, bool local_authority)
	{
		std::string error;
		const bool result = m_entities.apply_npc_state(state, local_authority, error);
		if (!result)
		{
			std::scoped_lock lock(m_cache_mutex);
			m_diagnostic = std::move(error);
	...
	void native_runtime::remove_npc_state(std::uint64_t npc_id, std::uint32_t generation)
	{
		m_entities.remove_npc_state(npc_id, generation);
	}

	bool native_runtime::apply_environment_state(const protocol::EnvironmentState &state, bool apply_weather)
	{
		if (!is_valid_environment_state(state))
		{
			std::scoped_lock lock(m_cache_mutex);
			m_diagnostic = "Native environment synchronization received an invalid state.";
			KCD2Online_JOIN_TRACE("join.environment.apply.failed", m_diagnostic);
			return false;
		}
		clear_script_global(environment_apply_result);
		const auto script = std::format("{0}=false; "
		...
		if (!execute_script(script) || !take_script_global_bool(environment_apply_result, calendar_applied) || !calendar_applied)
		{
			std::scoped_lock lock(m_cache_mutex);
			m_diagnostic = "Native Calendar world-time synchronization failed.";
			KCD2Online_JOIN_TRACE("join.environment.apply.failed", m_diagnostic);
			return false;
		}
		if (apply_weather)
		{
			const auto transition_ms = state.weather_revision() > m_native_weather_revision ? state.weather_transition_ms() : 0U;
			const auto command = std::format("cheat_set_weather id:{} transition:{:.3f}", state.weather_id(), static_cast<double>(transition_ms) / 1000.0);
			if (!execute_console_command(command.c_str()))
			{
				std::scoped_lock lock(m_cache_mutex);
				m_diagnostic = "Native environment synchronization could not apply weather.";
				KCD2Online_JOIN_TRACE("join.environment.apply.failed", m_diagnostic);
				return false;
			}
			m_native_weather_revision = state.weather_revision();
		}
		KCD2Online_JOIN_TRACE("join.environment.apply.ok",
		                      std::format("revision={} world_time_seconds={} weather_revision={} "
		                                  "apply_weather={}",
		                                  state.revision(),
		                                  state.world_time_seconds(),
	...
	bool native_runtime::set_home_marker(const std::optional<protocol::PropertyHomeMarker> &marker)
	{
		if (marker
		    && (marker->property_id().empty() || marker->level_id().empty() || !marker->has_position() || marker->entity_guid() == 0
		        || !std::isfinite(marker->position().x()) || !std::isfinite(marker->position().y())
		        || !std::isfinite(marker->position().z())))
		{
			return false;
		}
		if (m_home_marker && marker && m_home_marker->SerializeAsString() == marker->SerializeAsString())
		{
			return true;
		}
		remove_home_marker();
	...
	void native_runtime::refresh_home_marker()
	{
		if (!m_home_marker || m_native_home_mark || !sandbox_active()
		    || canonical_level_id(current_level_id()) != canonical_level_id(m_home_marker->level_id()))
		{
			return;
		}
		...
		KCD2Online_JOIN_TRACE("property.home-marker.applied",
		                      std::format("property_id=\"{}\" entity_guid={} entity_id={} name=\"{}\"",
	...
	void native_runtime::remove_home_marker()
	{
		...
		__try
		{
			...
			if (current && current == m_native_home_map)
			{
				...
				{
					current->SetPoiMarkersVisible(wh::guimodule::E_MarkType::Home, false);
				}
			}
#ifdef _WIN32
		}
	...
	bool native_runtime::apply_authoritative_profile(const protocol::PlayerProfile &profile)
	{
		...
		if (const auto current = m_profiles.capture(capture_error); current && same_native_profile_state(*current, profile))
		{
	...
	bool native_runtime::respawn_local_player(const protocol::TransformState &spawn)
	{
		if (!is_finite_transform(spawn))
		{
			return false;
		...
		const auto before_identity = capture_player_respawn_identity(before_player);
		if (!before_identity.complete())
		{
			return fail_respawn("Native respawn requires an existing client Entity, Actor, and Soul.");
		}
		auto *framework = CCryAction::GetInstance();
		if (!framework || !before_player.actor->IsPlayer() || framework->GetClientEntity() != before_player.entity
		    || framework->GetClientActor() != before_player.actor)
		{
			return fail_respawn("Native respawn rejected an inconsistent local-player ownership graph.");
		}
		KCD2Online_JOIN_TRACE("join.respawn.native.begin",
		                      std::format("entity_id={} entity={} actor={} soul={} health={:.3f} "
		                                  "dead={} spawn=({:.3f},{:.3f},{:.3f})",
		...
		const auto revive_dispatched = execute_script("KCD2Online_RespawnRevived=false; "
		...
		bool revived{};
		if (!revive_dispatched || !take_script_global_bool(revive_result, revived) || !revived)
		{
			return fail_respawn("Native C_Player revive did not reach a living, conscious state.");
		}

		const auto revived_player         = m_entities.player();
		const auto revived_identity       = capture_player_respawn_identity(revived_player);
		const auto revive_identity_result = validate_player_respawn_identity(before_identity, revived_identity);
		if (revive_identity_result != player_respawn_identity_result::stable)
		{
			return fail_respawn(std::format("Native revive violated player identity: {}.", to_string(revive_identity_result)));
		}
		...
		const auto view_dispatched = execute_script("KCD2Online_RespawnSettled=false; "
		                                            "if player and player.actor then "
		...
		bool settled{};
		if (!view_dispatched || !take_script_global_bool(settled_result, settled) || !settled)
		{
			return fail_respawn("Native player did not remain alive and conscious after spawn correction.");
		}

		const auto final_player          = m_entities.player();
		const auto final_identity        = capture_player_respawn_identity(final_player);
		const auto final_identity_result = validate_player_respawn_identity(before_identity, final_identity);
		if (final_identity_result != player_respawn_identity_result::stable)
		{
			return fail_respawn(std::format("Native respawn violated player identity after correction: {}.", to_string(final_identity_result)));
		}

		const auto game_over_hidden = guarded_hide_game_over();
		KCD2Online_JOIN_TRACE("join.respawn.native.complete",
		                      std::format("entity_id={} actor={} soul={} health={:.3f} "
		...
		{
			std::scoped_lock lock(m_cache_mutex);
			m_diagnostic = "Native player revived, but the game-over fader could not be hidden.";
		}
		return true;
	}
	...
	{
		constexpr std::string_view query = "KCD2Online_PlayerLaying = player and player.IsLaying "
		...
		bool result{};
		return take_script_global_bool("KCD2Online_PlayerLaying", result) && result;
	}
	...
	bool native_runtime::play_emote(std::string_view fragment)
	{
		...
		const auto script = std::format("local e=System.GetEntity({}); if e and e.human and "
		...
		m_explicit_animation_ends_at = std::chrono::steady_clock::now() + std::chrono::milliseconds(emote->duration_ms);
		return true;
	}

	void native_runtime::show_multiplayer_notice(std::string_view message)
	{
		const auto script = "if Game and Game.SendInfoText then "
		                    "Game.SendInfoText("
		                    + lua_string(message) + ", false) end";
	...
	void native_runtime::receive_voice(const protocol::ServerVoiceFrame &frame)
	{
		m_voice.receive(frame);
	}

	bool native_runtime::set_player_voice_volume(player_id player, float volume) noexcept
	{
		return m_voice.set_player_volume(player, volume);
	}
	...
	std::optional<protocol::TransformState> native_runtime::local_transform() const
	{
		std::scoped_lock lock(m_cache_mutex);
		return m_local_transform;
	}

	std::optional<protocol::AvatarDescriptor> native_runtime::local_avatar_visual() const
	{
		std::string error;
		return m_profiles.capture_avatar_visual(error);
	}

	bool native_runtime::apply_local_correction(const protocol::TransformState &transform)
	{
		std::string error;
		...
		m_transform_sequence = std::max(m_transform_sequence, transform.sequence());
		return true;
	}
	...
	remote_avatar_sync_result native_runtime::sync_remote_players(std::span<const remote_avatar_snapshot> players)
	{
		...
		m_remote_backend.set_epoch(m_epoch.load(std::memory_order_acquire));
		...
		for (const auto &player : players)
		{
			...
			voice_players.push_back(
			    {player.id, m_remote_backend.entity_id_for(player.id), player.transform.position(), player.transform.velocity()});
		}
		m_voice.update_players(voice_players);
		KCD2Online_JOIN_TRACE(result.success ? "join.remote-sync.complete" : "join.remote-sync.failed",
		                      std::format("success={} degraded={} spawned={} updated={} removed={} "
		                                  "error=\"{}\" diagnostic=\"{}\"",
		                                  result.success,
		                                  result.degraded,
		                                  result.spawned,
	...
	void native_runtime::invalidate_epoch_on_game_thread()
	{
		...
		m_native_activity_kind         = protocol::PLAYER_ACTIVITY_KIND_NONE;
		m_activity_end_pending         = false;
		const auto expected_transition = m_expected_epoch_transition.load(std::memory_order_acquire);
		...
		{
			m_sandbox_progress.phase = sandbox_phase::loading;
			m_diagnostic             = "Expected native world transition invalidated the old runtime epoch.";
			KCD2Online_JOIN_TRACE("join.world.epoch.expected", std::format("new_epoch={} target_level={}", m_epoch.load(std::memory_order_acquire), m_world_start_level_id));
		}
	}

	void native_runtime::refresh_cached_state()
	{
		const auto multiplayer_requested = m_multiplayer_requested.load(std::memory_order_acquire);
		std::uint64_t capabilities       = runtime_capability_kcse | runtime_capability_game_thread;
		std::string level;
		...
		const auto system_state_available = read_system_global_state(system_global_state);
		const auto level_load_complete = system_state_available && system_global_state == system_global_state_level_load_complete;

		auto *framework          = CCryAction::GetInstance();
		const auto native_player = m_entities.player();
		auto *entity             = native_player.entity;
		auto *framework_actor    = framework ? framework->GetClientActor() : nullptr;
		...
		{
			transition_blocker =
			    system_state_available ? std::format("Native level loading is still active (system state {}).", system_global_state) : "Native level-loading state is not available yet.";
		}
		else if (player_module)
		{
			transition_safe = true;
			if (player_module->m_pFastTravel && player_module->m_pFastTravel->IsFastTraveling())
			{
				transition_safe    = false;
				transition_blocker = "Finish or cancel fast travel before connecting.";
			}
			else if (player_module->m_pMinigameManager)
			{
				for (const auto &[user_id, session] : player_module->m_pMinigameManager->m_sessions)
				{
					(void)user_id;
					if (session && !session->IsFinished())
					{
						transition_safe    = false;
						transition_blocker = "Exit the active minigame before connecting.";
						break;
					}
				}
			}
		...
		{
			transition_blocker = "The native PlayerModule is not ready for multiplayer.";
		}
		if (environment_runtime_available())
		{
			capabilities |= runtime_capability_environment;
		}
		KCD2Online_JOIN_TRACE("join.runtime.cached-state.precheck",
		                      std::format("multiplayer_requested={} framework={} entity={} "
		                                  ...
		                                  system_state_available ? std::to_string(system_global_state) : "unavailable",
		                                  level_load_complete));
		...
		bool profile_ready{};
		bool avatar_ready{};
		if (multiplayer_requested && level_load_complete && (capabilities & runtime_capability_local_player) != 0)
		{
			if (m_probe_complete)
			{
		...
		KCD2Online_JOIN_TRACE("join.runtime.capability-components",
		                      std::format("transform_ready={} profile_ready={} avatar_ready={} "
		...
		if (multiplayer_requested && level_load_complete && (capabilities & runtime_capability_local_player) != 0 && !m_preparation_active)
		{
			m_remote_backend.reset_active_probe();
			m_probe_transform_verified = false;
			m_probe_complete           = false;
		...
		if (multiplayer_requested && level_load_complete && transform && profile_ready && avatar_ready && !m_probe_complete && !m_probe_failed)
		{
			if (!m_probe_transform_verified)
			{
				std::string probe_error;
				if (!m_entities.write_transform(entity, *transform, probe_error))
				{
					m_probe_failed = true;
					m_probe_error  = "active identical SetWorldTM probe failed: " + probe_error;
					KCD2Online_JOIN_TRACE("join.runtime.probe.identical-transform.failed", m_probe_error);
				}
				else
				{
					m_probe_transform_verified = true;
					KCD2Online_JOIN_TRACE("join.runtime.probe.identical-transform.ok", "SetWorldTM write/readback verified");
				}
			}
			if (m_probe_transform_verified && !m_probe_failed)
			{
				std::string probe_error;
				switch (m_remote_backend.poll_active_probe(*transform, probe_error))
				{
				case native_remote_avatar_backend::active_probe_result::succeeded:
					m_probe_complete = true;
					KCD2Online_JOIN_TRACE("join.runtime.probe.remote-avatar.ok", "Actor/Soul/Inventory/Equipment probe succeeded");
					break;
				case native_remote_avatar_backend::active_probe_result::failed:
					m_probe_failed = true;
					m_probe_error  = "active native Actor/Soul/Inventory/Equipment "
					                 "probe failed: "
					                + probe_error;
					KCD2Online_JOIN_TRACE("join.runtime.probe.remote-avatar.failed", m_probe_error);
					break;
				case native_remote_avatar_backend::active_probe_result::pending: break;
				}
			}
		}
		if (multiplayer_requested && m_preparation_active && !m_probe_complete && !m_probe_failed && ++m_preparation_frames > 900)
		{
			m_probe_failed = true;
			if (!transform)
			{
				m_probe_error = "Native local-player transform was not ready in time.";
			}
			else if (!profile_ready)
			{
				m_probe_error = profile_error.empty() ? "Native player profile was not ready in time." : std::move(profile_error);
			}
			...
			{
				m_probe_error = "Native multiplayer capability probe timed out.";
			}
		}
		if (multiplayer_requested && m_probe_complete)
		{
			capabilities |= runtime_capability_transform_write | runtime_capability_sandbox | runtime_capability_entity_isolation | runtime_capability_equipment | runtime_capability_profile_capture | runtime_capability_profile_apply | runtime_capability_profile_qam | runtime_capability_remote_avatar | runtime_capability_npc_sync;
		}
		if (player_module)
		{
			capabilities |= runtime_capability_transition_gate;
		}
		if (!m_address_library.empty() && !m_address_library_distribution.empty() && m_address_library_format != 0 && m_address_library_entries != 0
		    && m_address_library_sha256.size() == 64)
		{
		...
		auto *environment = SSystemGlobalEnvironment::GetInstance();
		KCD2Online_JOIN_TRACE("join.runtime.entity-system.state",
		                      std::format("environment={} entity_system={} console={} local_entity={}",
		                                  static_cast<void *>(environment),
		                                  environment ? static_cast<void *>(environment->pEntitySystem) : nullptr,
		                                  environment ? static_cast<void *>(environment->pConsole) : nullptr,
		                                  static_cast<void *>(entity)));
		...
		if (transform)
		{
			if (!m_explicit_animation_fragment.empty())
			{
				...
				{
					sampled_animation_fragment = m_explicit_animation_fragment;
				}
				else
				{
					m_explicit_animation_fragment.clear();
			...
			if (m_local_transform && m_local_transform_sampled_at != std::chrono::steady_clock::time_point{})
			{
				...
				if (elapsed >= 0.001F && elapsed <= 0.25F)
				{
					...
					*locomotion->mutable_facing_direction() = facing;
					...
					locomotion->set_strafing(std::abs(local->x()) > 0.2F && std::abs(local->y()) > 0.2F);

					const auto old_facing = facing_from_rotation(m_local_transform->rotation());
					const auto yaw        = std::atan2(facing.x(), facing.y());
					const auto old_yaw    = std::atan2(old_facing.x(), old_facing.y());
					auto yaw_delta        = std::remainder(yaw - old_yaw, 2.0F * std::numbers::pi_v<float>);
			...
			{
				auto *locomotion                        = transform->mutable_locomotion();
				*locomotion->mutable_facing_direction() = facing_from_rotation(transform->rotation());
		...
		if ((capabilities & runtime_capability_local_player) == 0)
		{
			if (!m_world_start_bootstrap)
			{
				m_diagnostic = "No native player is loaded; the server bootstrap can start "
				               "a save-free world.";
			}
		}
		...
		{
			m_diagnostic = "The native player transform is not readable through the "
		...
		{
			m_diagnostic = "Active native multiplayer ABI probe is running.";
		}
		else
		{
			m_diagnostic = profile_error.empty() ? m_remote_backend.diagnostic() : std::move(profile_error);
			if ((capabilities & runtime_capability_remote_avatar) != 0)
			{
				m_diagnostic = "All native multiplayer runtime capabilities are ready.";
			}
		}
	}
	...
	{
		auto *framework = CCryAction::GetInstance();
		KCD2Online_JOIN_TRACE("join.sandbox.unload.begin",
		                      std::format("reason=\"{}\" framework={} game_context={}",
		                                  reason,
		                                  static_cast<void *>(framework),
		                                  framework ? static_cast<void *>(framework->m_pGameContext) : nullptr));
		{
			std::scoped_lock lock(m_cache_mutex);
		...
		m_multiplayer_requested.store(false, std::memory_order_release);
		m_expected_epoch_transition.store(false, std::memory_order_release);
		KCD2Online_CRITICAL_TRACE("join.sandbox.unload.requested", "native world teardown requested");
	}
	...
	void native_runtime::queue_native_unload_if_safe()
	{
		...
		if (!level_load_complete)
		{
			...
			if (should_log)
			{
				KCD2Online_JOIN_TRACE("join.sandbox.unload.deferred", "Native unload is waiting for the engine to report LEVEL_LOAD_COMPLETE.");
			}
			return;
		}
		...
		if (!teardown_started)
		{
			...
			if (!teardown_started)
			{
				KCD2Online_CRITICAL_TRACE("join.sandbox.unload.teardown.begin", "dropping multiplayer ownership before engine world teardown");
				(void)execute_script("if player and player.EnableFastTravel then "
				                     "player:EnableFastTravel(true) end");
				m_remote_avatars.abandon_world();
				m_remote_backend.abandon_world();
				KCD2Online_CRITICAL_TRACE("join.sandbox.unload.avatars.abandoned", "remote actors left intact for CryEngine context destruction");
				m_entities.restore_world();
				m_profiles.reset();
				KCD2Online_CRITICAL_TRACE("join.sandbox.unload.teardown.complete", "world ownership restored; waiting for next engine frame");
				return;
			}
		}
		...
		if (framework && framework->m_pGameContext)
		{
			...
			KCD2Online_CRITICAL_TRACE("join.sandbox.unload.command.begin",
			                          std::format("framework={} game_context={}",
			                                      static_cast<void *>(framework),
			...
			KCD2Online_CRITICAL_TRACE(queued ? "join.sandbox.unload.command.queued" : "join.sandbox.unload.command.failed", queued ? "Deferred native disconnect and frontend return were queued." : "Deferred native disconnect could not be queued.");
		}
		else
		{
			KCD2Online_JOIN_TRACE("join.sandbox.unload.command.skipped", "framework or game context is nil");
		}
	}
	...
	{
		auto *framework = CCryAction::GetInstance();
		return !framework || (!framework->m_pGameContext && framework->GetClientEntity() == nullptr);
	}
	...
	void native_runtime::finish_native_unload_if_complete()
	{
		...
		KCD2Online_CRITICAL_TRACE("join.sandbox.unload.complete", "Native disconnect completed; CryEngine owns the frontend transition.");
	}
} // namespace kcd2o::kcse
