...
namespace kcd2o::kcse::native_keybinds
{
	namespace
	{
		...
		using parse_keybinds = void (*)(
			wh::playermodule::C_Keybinds *, IXmlNode **);
		...
		Offsets::IActionMapManager *action_map_manager() noexcept
		{
			...
			return framework
			    ? reinterpret_cast<Offsets::IActionMapManager *>(
			          framework->m_pActionMapManager)
			    : nullptr;
		}
		...
		public:
			void OnAction(
			    const Offsets::SActionId &action,
			    int activation_mode,
			    float) override
			{
		    ...
		    const char *value)
		{
			for (int index{}; index < parent->getChildCount(); ++index)
			{
				...
				if (!child)
					continue;
				if (std::strcmp(child->getTag(), tag) == 0
				    && std::strcmp(child->getAttr(attribute), value) == 0)
					return child;
				child->Release();
			}
			return nullptr;
		...
		void add_action_reference(
		    IXmlNode *superaction,
		...
		void add_superaction(
		    IXmlNode *root,
		    std::string_view name,
		    const char *label,
		    const char *input)
		{
			if (auto *existing = find_child(
			        root, "superaction", "name", name.data()))
			{
				existing->Release();
				return;
			}
		...
		bool add_player_actions()
		{
			...
			if (!manager || !environment || !environment->pSystem)
				return false;
			auto *player = static_cast<Offsets::IActionMap *>(
				manager->GetActionMap("player"));
			if (!player)
				return false;
			...
			constexpr char actions[] =
				...
				"<action name=\"kcd2o_emote\" onPress=\"1\" onRelease=\"1\" keyboard=\"_keybinds_ref_\" noModifiers=\"1\"/>"
				"</actionmap>";
			IXmlNode *root{};
			environment->pSystem->LoadXmlFromBuffer(
				&root, actions, sizeof(actions) - 1, 0, 1);
			if (!root)
				return false;
		...
		void inject_keybinds(
		    wh::playermodule::C_Keybinds *keybinds,
		    IXmlNode **root)
		{
			if (!root || !*root)
			{
			...
			if (auto *group = find_child(
			        patched, "ui_group", "name", "kcd2online"))
			{
				group->Release();
			}
			else
			...
			add_superaction(
				patched, chat_action, "kcd2o_keybind_chat", "enter");
			add_superaction(
				patched, voice_action, "kcd2o_keybind_voice", "v");
			add_superaction(
				patched, emote_action, "kcd2o_keybind_emote", "g");

			if (auto *conflict = find_child(
			        patched, "conflict", "name", "general"))
			{
				for (const auto name : {
				         chat_action, voice_action, emote_action})
				{
			...
			*root = patched;
			...
			g_original_parse(keybinds, root);
			g_available.store(
				actions_ready && register_listener(),
	...
	bool install()
	{
		g_original_parse = REL::Relocation<>{
			REL::ID(33537), 0x73}.write_call<5>(inject_keybinds);
